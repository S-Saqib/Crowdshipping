...
public class ServiceQueryProcessor {
    ...
    private long ignoreIOSimulationTimeBL;
    ...
    ArrayList<Trajectory> retrieveTrajsSQ(ArrayList<Node> summaryNodes){
    ...
    ArrayList<Trajectory> retrieveTrajsSQ_tPruning(ArrayList<Node> summaryNodes, PacketRequest pktReq){
        HashSet<Node> baseQuadTreeNodes = new HashSet<>();
        QuadTree baseQuadTree = quadTrajTree.getQuadTree();
        for (Node node : summaryNodes){
            double xMin = node.getX();
            ...
            Node[] intersectingNodes = baseQuadTree.searchIntersect(xMin, yMin, xMax, yMax);
            for (Node n : intersectingNodes){
                // no SQ temporal bitmap filtering
                // steps for pruning: 1. keep time bitmap with SQ nodes
                // 2. check for time bucket intersection of base quadtree node and summary node
                // 3. prune if the time windows are disjoint
        ...
        // trajectory retrieval
        int minTimeBucketId = quadTrajTree.getQuadTree().getTimeIndex(pktReq.getSrcTimeInSec());
        int maxTimeBucketId = quadTrajTree.getQuadTree().getTimeIndex(pktReq.getDestTimeInSec());
        HashSet<Trajectory>retrievedTrajs = new HashSet<>();
        for (Node node : baseQuadTreeNodes){
            HashMap<Integer, HashSet<Object>> timeBucketToDiskBlockMap = node.getTimeBucketToDiskBlockIdMap();
            if (timeBucketToDiskBlockMap == null) continue;
            for (HashMap.Entry<Integer, HashSet<Object>> entry : timeBucketToDiskBlockMap.entrySet()){
                ...
                //QG temporal pruning
                int timeBucketId = entry.getKey();
                if (timeBucketId < minTimeBucketId || timeBucketId > maxTimeBucketId) continue;
    ...
    ArrayList<Trajectory> retrieveTrajsBL(ArrayList<Node> summaryNodes){
        HashSet<Node> baseQuadTreeNodes = new HashSet<>();
        QuadTree baseQuadTree = quadTrajTree.getQuadTree();
        for (Node node : summaryNodes){
            double xMin = node.getX();
            ...
            for (HashMap.Entry<Integer, HashSet<Object>> entry : timeBucketToDiskBlockMap.entrySet()){
                ...
                for (Object diskBlockId : allDiskBlocks){
                    ...
                    for (String trajId : allTrajIds){
                        ...
                        */
                        // let us use pre calculated baseline block id directly
                        long ignoreIOsimulationTimeBLStart = System.nanoTime();
                        long quadtreeBasedDiskBlockId = trajStorage.getTransformedTrajectoryById(trajId).getTransformedPointList().first().getqNodeIndex();
                        // assumption : at most 32 trajs in each block as node capacity is 32 points (each traj size around 128B, block size 4k => trajs per block = 32)
                        ArrayList<Long> blDiskBlockList = quadTrajTree.getQuadTree().getqNodeIndexToNodeMap().get(quadtreeBasedDiskBlockId).getBaselineBlockIds();
                        for (Long baselineBlockId : blDiskBlockList){
                            diskBlockSet.add((baselineBlockId));   
                        }
                        long ignoreIOsimulationTimeBLEnd = System.nanoTime();
                        ignoreIOSimulationTimeBL += (ignoreIOsimulationTimeBLEnd - ignoreIOsimulationTimeBLStart);
                        retrievedTrajs.add(traj);
                    }
                }
            }
    ...
    ArrayList<Trajectory> retrieveTrajsBL_tPruning(ArrayList<Node> summaryNodes, PacketRequest pktReq){
        HashSet<Node> baseQuadTreeNodes = new HashSet<>();
        QuadTree baseQuadTree = quadTrajTree.getQuadTree();
        for (Node node : summaryNodes){
            double xMin = node.getX();
            ...
            Node[] intersectingNodes = baseQuadTree.searchIntersect(xMin, yMin, xMax, yMax);
            baseQuadTreeNodes.addAll(Arrays.asList(intersectingNodes));
            // no SQ temporal filtering
            // check retrieveTrajsSQ_tPruning comments if this is to be done
            //if (!baseQuadTreeNodes.contains(n)) System.out.println(n);
        }
        // System.out.println("Retrieved base quad tree nodes = " + baseQuadTreeNodes.size());
    ...
    // difference with construcTrajGraphWithTime: graph is directional based on trajGraphNode time
    // graph should essentially be the same
    ...
    public ArrayList<TrajGraphNode> deliverSQ(PacketDeliveryQuery pktDeliveryQuery, int temporalProcess){
        PacketRequest pktRequest = pktDeliveryQuery.getPacketRequest();
        long algoStartTime = System.nanoTime();
        // a single function for summary node retrieval for all exp, no need to change it as temporal property is absent here
        ...
        // trajectory retrieval without temporal pruning
        ArrayList<Trajectory> reducedTrajs;
        ...
        int trajsRetrieved = 0;
        TrajGraph trajGraph;
        ArrayList<TrajGraphNode> bestDeliverers;
        if (temporalProcess == 0){
            reducedTrajs = retrieveTrajsSQ(summaryNodes);
            ...
            reducedTrajs = retrieveTrajsSQ_tPruning(summaryNodes, pktRequest);
            trajsRetrieved = reducedTrajs.size();
            trajGraph = constructTrajGraph_joinWithTimeDirectional(reducedTrajs);
            bestDeliverers = traverseToDeliver_AStarWithDuration(trajGraph, pktRequest);
        }
            ...
            // do nothing
            reducedTrajs = new ArrayList<>();
            trajsRetrieved = reducedTrajs.size();
            trajGraph = new TrajGraph();
            bestDeliverers = new ArrayList<>();
        }
        algoEndTime = System.nanoTime();
        runtimeInSec = (algoEndTime - algoStartTime)/1.0e9;
        ...
        System.out.print(isDelivered + "\t" + (isDelivered?this.deliveryCost:-1) + "\t" + this.deliveryTimeInSec + "\t" + runtimeInSec + "\t" + this.diskBlockSet.size() + "\t");
        this.deliveryCost = Double.MAX_VALUE;
        this.deliveryTimeInSec = -1;
    ...
    public ArrayList<TrajGraphNode> deliverBL(PacketDeliveryQuery pktDeliveryQuery, int temporalProcess){
        int io = 0;
        ignoreIOSimulationTimeBL = 0;
        PacketRequest pktRequest = pktDeliveryQuery.getPacketRequest();
        long algoStartTime = System.nanoTime();
        //ArrayList<Node> summaryNodes = retrieveSummaryNodesModified(pktRequest);
        ...
        ArrayList<TrajGraphNode> bestDeliverers;
        if (temporalProcess == 0){
            reducedTrajs = retrieveTrajsBL(rangeQueryNodes);
            trajsRetrieved = reducedTrajs.size();
            trajGraph = constructTrajGraph_join(reducedTrajs);
            bestDeliverers = traverseToDeliver_AStar(trajGraph, pktRequest);
        }
            ...
            reducedTrajs = retrieveTrajsBL_tPruning(rangeQueryNodes, pktRequest);
            trajsRetrieved = reducedTrajs.size();
            trajGraph = constructTrajGraph_joinWithTimeDirectional(reducedTrajs);
            bestDeliverers = traverseToDeliver_AStarWithDuration(trajGraph, pktRequest);
        }
        ...
        boolean isDelivered = !bestDeliverers.isEmpty();
        // only doubling the query rectangle size once, irrespective of whether pkt can be delivered or not
        //if (isDelivered){
        if (true){
            ...
            if (!isDelivered){
                ...
                if (temporalProcess == 0){
                    ...
                    QuadTree quadTree = quadTrajTree.getQuadTree();
                    int minTimeBucketId = quadTree.getTimeIndex(pktRequest.getSrcTimeInSec());
                    int maxTimeBucketId = quadTree.getTimeIndex(pktRequest.getDestTimeInSec());
                    for (Trajectory traj : allTrajs){
                        int firstPointTimeBucket = quadTree.getTimeIndex(traj.getPointList().first().getTimeInSec());
                        int lastPointTimeBucket = quadTree.getTimeIndex(traj.getPointList().last().getTimeInSec());
                        if (firstPointTimeBucket < minTimeBucketId || firstPointTimeBucket > maxTimeBucketId) continue;
                        if (lastPointTimeBucket < minTimeBucketId || lastPointTimeBucket > maxTimeBucketId) continue;
                        reducedTrajs.add(traj);
                    }
                    trajsRetrieved = reducedTrajs.size();
                    trajGraph = constructTrajGraph_joinWithTimeDirectional(reducedTrajs);
                    bestDeliverers = traverseToDeliver_AStarWithDuration(trajGraph, pktRequest);
        ...
        double runtimeInSec = ((algoEndTime - algoStartTime)-ignoreIOSimulationTimeBL)/1.0e9;
        //System.out.print(isDelivered + "\t" + this.deliveryCost + "\t" + runtimeInSec+ "\t" + trajsRetrieved + "\t");
        System.out.print(isDelivered + "\t" + (isDelivered?this.deliveryCost:-1) + "\t" + this.deliveryTimeInSec + "\t" + runtimeInSec+ "\t" + io +  "\t");
        this.deliveryCost = Double.MAX_VALUE;
        this.deliveryTimeInSec = -1;
        this.diskBlockSet.clear();
        return bestDeliverers;
    ...
    public ArrayList<TrajGraphNode> deliverPacketModifiedWithJoin(PacketDeliveryQuery pktDeliveryQuery){
        ...
        //System.out.println("\n--##--%%--$$--%%--##--");
        //System.out.println("Summary node overlap processing : " + runtimeInSec + " sec");
        ArrayList<Trajectory> reducedTrajs = retrieveTrajsSQ(summaryNodes);
        //algoEndTime = System.nanoTime();
        //runtimeInSec = (algoEndTime - algoStartTime)/1.0e9;
        //System.out.println("Trajectory retrival : " + runtimeInSec + " sec");
        ...
        boolean isDelivered = !bestDeliverers.isEmpty();
        //System.out.print(isDelivered + "\t" + this.deliveryCost + "\t" + runtimeInSec + "\t" + trajsRetrieved + "\t");
        System.out.print(isDelivered + "\t" + (isDelivered?this.deliveryCost:-1) + "\t" + runtimeInSec + "\t" + this.diskBlockSet.size() + "\t" + this.deliveryTimeInSec + "\t");
        this.deliveryCost = Double.MAX_VALUE;
        this.deliveryTimeInSec = -1;
        this.diskBlockSet.clear();
    ...
    public ArrayList<TrajGraphNode> deliverPacketBaselineWithJoin(PacketDeliveryQuery pktDeliveryQuery){
        ...
        ArrayList<Node> rangeQueryNodes = retrieveRangeQueryNodes(pktRequest);
        // a single node should suffice, but trying to reuse previously written function
        ArrayList<Trajectory> reducedTrajs = retrieveTrajsBL(rangeQueryNodes);
        int trajsRetrieved = reducedTrajs.size();
        TrajGraph trajGraph;
        ArrayList<TrajGraphNode> bestDeliverers;
        trajGraph = constructTrajGraph_join(reducedTrajs);
        ...
        double runtimeInSec = (algoEndTime - algoStartTime)/1.0e9;
        //System.out.print(isDelivered + "\t" + this.deliveryCost + "\t" + runtimeInSec+ "\t" + trajsRetrieved + "\t");
        System.out.print(isDelivered + "\t" + (isDelivered?this.deliveryCost:-1) + "\t" + runtimeInSec+ "\t" + io +  "\t" + this.deliveryTimeInSec + "\t");
        this.deliveryCost = Double.MAX_VALUE;
        this.deliveryTimeInSec = -1;
        this.diskBlockSet.clear();
        traverseToDeliver(trajGraph, pktRequest);
    ...
    ArrayList<TrajGraphNode> traverseToDeliver_AStarWithDuration(TrajGraph trajGraph, PacketRequest pktRequest){
        ...
        while(!explorableNodes.isEmpty()) {
            ...
            // comment out the following line for ignoring temporal info, checked: worked for 20 pkts
            ...
            for (TrajGraphNode node : neighbors){
                ...
                // the node to be enqueued is keeper, assuming keeper is reached instantaneously
                // later we can add a time based on distance from curNode stop to node stop
                // from a study (pdf in dropbox), melbourne avg speed is 63.5 km/h
                // it can be used to estimate time to reach keeper
    ...
    // consider the remaining portion as legacy code, may be in use but is not directly required for our purpose
    ...
    // difference with construcTrajGraph: only time info is kept with trajgraph nodes
    ...
    public ArrayList<TrajGraphNode> deliverPacketAllTrajWithJoin(PacketDeliveryQuery pktDeliveryQuery){
        ...
        double runtimeInSec = (algoEndTime - algoStartTime)/1.0e9;
        boolean isDelivered = !bestDeliverers.isEmpty();
        System.out.print(isDelivered + "\t" + (isDelivered?this.deliveryCost:-1) + "\t" + runtimeInSec+ "\t" + trajsRetrieved + "\t" + this.deliveryTimeInSec + "\t");
        this.deliveryCost = Double.MAX_VALUE;
        this.deliveryTimeInSec = -1;
        this.diskBlockSet.clear();
        traverseToDeliver(trajGraph, pktRequest);
